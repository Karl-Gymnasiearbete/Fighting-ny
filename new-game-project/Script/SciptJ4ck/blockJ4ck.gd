...
var just_entered := false  # Add this
...
func enter() -> void:
	...
	just_entered = true  # Set flag so we skip the first frame check
	is_blocking = true
...
func Physics_Update(delta: float) -> void:
	...
	# Skip exit check on the first frame so is_action_pressed has time to register
	if just_entered:
		just_entered = false
		return
